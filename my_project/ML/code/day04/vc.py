...
_, test_scores = ms.validation_curve(model, train_x, train_y, 'n_estimators', n_estimators, cv=5)
test_means1 = test_scores.mean(axis=1)
model = se.RandomForestClassifier(n_estimators=200, random_state=7)
max_depth = np.arange(1, 11)
_, test_scores = ms.validation_curve(model, train_x, train_y, 'max_depth', max_depth, cv=5)
...
mp.grid(linestyle=':')
mp.plot(n_estimators, test_means1, 'o-', c='dodgerblue', label='Testing')
mp.legend()
...
mp.tick_params(labelsize=10)
mp.grid(linestyle=':')
mp.plot(max_depth, test_means2, 'o-', c='orangered', label='Testing')
mp.legend()
mp.show()
